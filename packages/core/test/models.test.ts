...
const ORIGINAL_DISABLE_FETCH = process.env["OPENCODE_DISABLE_MODELS_FETCH"]
beforeAll(() => {
...
afterAll(() => {
  ...
  if (ORIGINAL_DISABLE_FETCH === undefined) delete process.env["OPENCODE_DISABLE_MODELS_FETCH"]
  else process.env["OPENCODE_DISABLE_MODELS_FETCH"] = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          process.env["OPENCODE_DISABLE_MODELS_FETCH"] = "0"
        }),
