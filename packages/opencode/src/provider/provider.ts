...
// ENG-12 / RULE-02: the platform gateway is the only provider the product ships, so the
// openai-compatible adapter is the only SDK that can be instantiated. Every other vendor
// adapter was removed; nothing resolves an adapter outside this table.
export const PROVIDER_NPM_ALLOWLIST = ["@ai-sdk/openai-compatible"] as const

const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  "@ai-sdk/openai-compatible": () => import("@ai-sdk/openai-compatible").then((m) => m.createOpenAICompatible),
}

export function isAllowedProviderNpm(npm: string) {
  return Object.hasOwn(BUNDLED_PROVIDERS, npm)
}
...
// The vendor login and env-var autoload registry is gone with the provider lock: the
// platform gateway is configured, never discovered. `dep` stays so the call site keeps its
// shape for upstream merges.
  ...
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        for (const [id, provider] of Object.entries(providers)) {
          ...
          for (const [modelID, model] of Object.entries(provider.models)) {
            ...
            // ENG-12 / RULE-02: a config or plugin may name any provider, but a model that
            // would need a removed vendor adapter is dropped here, so it is never selectable.
            if (!isAllowedProviderNpm(model.api.npm)) {
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        throw new Error(
          `Provider adapter "${model.api.npm}" is not available; this build only supports ${PROVIDER_NPM_ALLOWLIST.join(", ")}`,
        )
      } catch (e) {
